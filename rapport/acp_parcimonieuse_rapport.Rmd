...
Dans ce document, nous avons décrits deux méthodes en lien avec l'analyse en composantes principales parcimonieuse, soit la méthode SCoTLASS et la méthode SPCA. Il est pertinent de mentionner qu'une autre méthode a été développée, soit la méthode PMA [@Witten]. Cette dernière est basée sur une décomposition pénalisée de la matrice de covariance (ou corrélation). Il existe d'ailleurs une implémentation de cette méthode en **R** qu'on peut retrouver dans la librarie *PMA*.
