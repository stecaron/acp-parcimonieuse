...
Chaque composante principale peut être définit comme une combinaison linéaire des $p$ variables originales du jeu de données, donc les coefficients de saturation sur les composantes principales peuvent être également obtenus en faisant la régression de la composante principale sur ces $p$ variables. Après l'application de l'ACP sur le jeu de donnée, on reconstruit les coefficients par une régression ridge, ce qui veut dire que cette méthode dépend des résultats obtenus avec l'ACP.
...
Ensuite, la méthode rajoute une nouvelle pénalité $L_{1}$ à l’équation précédente, la pénalité de Lasso, ce qui donne un nouveau problème d'optimisation:
...
####Notes:
1. Les résultats de cet algorithme ne change pas vraiment à la variation de $\lambda$. En pratique, on choisi $\lambda$ un petit nombre positif pour éviter les problèmes potentiels de colinéarité dans X.
