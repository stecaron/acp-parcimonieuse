...
Chaque composante principale peut être définit comme une combinaison linéaire des $p$ variables originales du jeu de données, donc les coefficients de saturation sur les composantes principales peuvent être également être obtenus en faisant la régression de la composante principale sur ces $p$ variables. Après l'application de l'ACP sur le jeu de donnée, on reconstruit les coefficients par une régression ridge, ce qui veut dire que cette méthode dépend des résultats obtenus avec l'ACP.
...
Le deuxième élément dans la dernière équation, qui est contrôlé par un hyperparamètre $lambda$, permet de contrôler la régularisation effectuée sur la régression. Effectivement, plus il y a de coefficients non nuls dans la régression, plus l'erreur est élevée. Cela aura donc pour effet de ne pas garder de variables inutiles dans le modèle.
...
Ensuite, la méthode rajoute une nouvelle pénalité $L_{1}$; la pénalité de Lasso, à l’équation précédente, ce qui donne un nouveau problème d'optimisation:
...
####Notes:
1. Les résultats de cet algorithme ne change pas vraiment à la variation de $\lambda$. En pratique, on choisi $\lambda$ un petit nombre positive pour éviter les problèmes potentiels de colinéarité dans X.
...
On remarque à la figure 1 que la régularisation faite par la méthode SPCA est forte sur les composantes 4 à 6. Cela est effectivement contrôlé par l'hyperparamètre $\lambda_1$ définit un peu plus tôt. Pour les premières composantes, on voit qu'il y a un lien entre le nombres de coefficients nuls de la méthode SPCA et le nombre de coefficients faibles des autres méthodes. Cela veut donc dire que les coefficients qui ont été tiré vers $0$ sont probablement des coefficients "peu importants" et sont ceux que nous aurions exclut intuitivement de notre interprétation.
