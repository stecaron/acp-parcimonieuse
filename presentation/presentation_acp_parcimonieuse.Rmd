...
author: "StÃ©phane Caron/Sofia Harrouch"
date: '2018-03-28'
output:
...
- Soit X le jeu de données $X=(X_{1}, .., X_{p})^{T}$.
...
Soit $Y_k =\alpha'_k X$ la kième composante principale obtenue à partir d'une ACP (une combinaison linéaires des variables initiales).

Ces coefficients de saturation $\alpha_k$ peuvent aussi être obtenus en faisant une regression multiple de la composante principale sur les $p$ variables initiales ($Y_k = X\beta + \epsilon$).
...
Comparer la performance de l'ACP parcimonieuse par rapport à la méthode de rotation en terme de simplicité et d'interprétabilité.
